...
import {
  IDataPageDeliveryForWash,
  IDataQty,
  IDropdown,
  ListFilterDeliveryForWash,
} from './types/deliveryForWashTypes';
import {
  IError,
  IRule,
  typeVadilate,
...
export interface DeliveryForWashProps {
...
const DeliveryForWashScreen: React.FC<DeliveryForWashProps> = ({
  navigation,
  baseAction,
}) => {
  const isFocused = useIsFocused();
  const [dataPageInput, setdataPageInput] = useState<IDataPageDeliveryForWash>({
    dateRecord: date,
    workCenterId: '',
    mainProductCode: '',
    colorCode: '',
  ...
  const getDataDropDownByFilter = async (
    ...
    if (workCenterId != '' &&
      ...
      //api lay lenh sx
      let dataResponse = await CommonBase.getAsync<ResponseService>(ApiDeliveryForWash.LIST_FILTER
        + '?Type=' + FilterRecord.Workordercode
      ...
      if (typeof dataResponse !== 'string' && dataResponse != null && dataResponse.isSuccess === true) {
        ...
        if ((data.Workordercode).length == 1) {
          ...
          //api lay ma hang
          let dataResponse = await CommonBase.getAsync<ResponseService>(ApiDeliveryForWash.LIST_FILTER
            + '?Type=' + FilterRecord.Maincodeproduct
            + '&Workcenterid=' + workCenterId
            + '&Workordercode=' + dataDropdownWorkordercode[0].id
            , null)
          if (typeof dataResponse !== 'string' && dataResponse != null && dataResponse.isSuccess === true) {
            ...
            if ((data.mainProductCodeData).length > 0) {
              ...
              //api lay ma nho 
              let dataResponse = await CommonBase.getAsync<ResponseService>(ApiDeliveryForWash.LIST_FILTER
                + '?Type=' + FilterRecord.Subcodeproduct
                + '&Workcenterid=' + workCenterId
                + '&Workordercode=' + dataDropdownWorkordercode[0].id
                + '&Mainproductcode=' + dataDropdownMainCode[0].id
                , null)
    ...
    if (
      ...
      let dataResponse = await CommonBase.getAsync<ResponseService>(ApiDeliveryForWash.LIST_FILTER
        ...
        + '&Workcenterid=' + workCenterId
        + '&Workordercode=' + workOrderCode
        + '&Mainproductcode=' + mainCodeProduct
  ...
  const getDataQty = async (value: string) => {
    if (
      ...
      baseAction.setSpinnerReducer({isSpinner: true, textSpinner: ''});
      let dataResponse = await CommonBase.getAsync<ResponseService>(
        ApiDeliveryForWash.GET_INFO_DELIVERY_WASH +
          '?DateRecord=' +
          dataPageInput.dateRecord +
          '&Workcenterid=' +
          dataPageInput.workCenterId +
  ...
  const onPressHandleSubmit = async () => {
    ...
    if (checkData != null) {
      if (dataQty.QtyLK - checkData + +dataPageInput.qtyNow > dataQty.QtySX) {
        let msg = 'Số lượng gửi đi không được vượt tổng ra chuyền!';
    ...
    let dataResponse = await CommonBase.postAsync<ResponseService>(
      ApiDeliveryForWash.RECORD_DELIVERY_WASH,
      dataRequest,
    );
    if (
      typeof dataResponse !== 'string' &&
      ...
      <ScrollView
        ...
        {/* <View style={styles.header}>
                  <Text style={styles.textHeaderLabel}>
                      Thông tin phiếu giặt
                  </Text>
              </View> */}
        <View
          style={
            {
              // height: deviceHeight - styles.header.height - (deviceHeight - (deviceHeight * 57) / 100)
            }
          }>
          <View style={styles.content}>
            ...
            <View style={styles.textInputContent}>
              ...
              (
                ...
                <View style={styles.textInputContent}>
              <View style={{ width: 120 }}>
                <Text style={styles.label}>Màu/ Mùa:
                </Text>
              </View>
              <View style={{ width: deviceWidth - 100 - 20 }}>
                <Text style={styles.input}>{dataShowColor ?? ''}/ {dataShowSeason ?? ''}</Text>
              </View>
            </View>
                </>
              )
            ...
            <View style={styles.textInputContent}>
              <View style={{width: 120}}>
                <Text style={styles.label}>Số lượng gửi đi:</Text>
              </View>
              <View style={{width: deviceWidth - 100 - 20}}>
                <TextInput
        ...
        <View
          ...
          <View style={styles.content}>
            ...
            <View style={styles.textInputContent}>
              <View style={{width: 150, paddingLeft: 10}}>
                <Text style={styles.label}>Số lượng đã gửi:</Text>
              </View>
              <View style={{width: deviceWidth - 100 - 20}}>
                <TextInput
                  value={dataQty.QtyLK + ''}
...
export default connect(null, mapDispatchToProps)(DeliveryForWashScreen);
